...
<?php
	...
	function getReadmoreLink() 
	{
		...
		return '<a class="'.$css.'" href="'.$link.'" title="'.htmlspecialchars($title).'"><i class="fa fa-file-text-o"></i> '.JText::_('TM_READ_MORE').'</a>';
	}
	...
	function getCommentsLink()
	{
		...
		switch($style) {
			case -1:
				...
				break;
			default:
				return '<a class="'.$css.'" href="'.$link.'" title="'.htmlspecialchars($text).'"><i class="fa fa-comments-o"></i> '.$text.'</a>';
				break;
		}
	}
}