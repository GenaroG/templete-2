...
// Prepare the Icon List
	for($i=0;$i < count($ic);$i++)
	 ...
	 if ($params->get('font_icons') == 1){
	 	$filename = substr($vurl[$url[$i]], 0, -4);	 	
	 }
	 
// Output the Icon Links	
	 	if(($vimg[$ic[$i]]) != '') {		 	
			echo '<a style="margin:'.$margin.';" '. $nofollow .' href="'. $vimg[$ic[$i]]. '" target="'. $target .'">
				<i class="fa fa-'. $filename .'"></i><span>'. $title .'</span>
				<img src="'. $mod .'icons/'. $theme .'/'. $size .'/'. $vurl[$url[$i]] .' " alt="'. $title .'" '; 
			if($title == 'Feed') {
				echo 'title="'. $rsstext .'" /></a>';
			}else{ 
				echo 'title="'. $text .' '. $title .'" /></a>';
			}
		}
	} 
